...
int dw_main(void)
{
    ...
    s1switch = portB_is_switch_on(TA_SW1_2) << 1     //读取拨码开关
    		 | portC_is_switch_on(TA_SW1_3) << 2
    		 | portC_is_switch_on(TA_SW1_4) << 3
    		 | portC_is_switch_on(TA_SW1_5) << 4
		     | portC_is_switch_on(TA_SW1_6) << 5
    		 | portC_is_switch_on(TA_SW1_7) << 6
    		 | portC_is_switch_on(TA_SW1_8) << 7;


	if(inittestapplication(s1switch) == (uint32)-1)  //初始化dw1000错误
	{
		led_on(LED_ALL); //led_常亮
#if(OLED == 1)
